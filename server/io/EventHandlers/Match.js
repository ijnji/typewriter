...
function randomMatch() {
    ...
    for (var game in gameRooms){
        if (gameRooms[game].length === 1) {
            ...
            let players = this.io.sockets.adapter.rooms[game].sockets;
            let usersArr = [];
        _.forOwn(players, function (v, k) {
            usersArr.push(k);
        })
        ...
        this.io.sockets.in(game).emit('gameStart', { room: game, player1: player1, player2: player2 });
        console.log('gameStart emitted', game);
        wordEmitter.emitWords(game, this.io);
         }
     }
